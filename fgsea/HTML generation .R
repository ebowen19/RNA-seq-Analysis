source('~/Library/CloudStorage/Box-Box/Wu Lab/Project - statin/8. RNA-seq/Elizabeth/scripts/RNA-seq-Analysis/fgsea/functions.R')
library(knitr)
library(kableExtra)
library(rvest)
library(xml2)

main_folder <- '~/Library/CloudStorage/Box-Box/Wu Lab/Project - statin/8. RNA-seq/Elizabeth/LW15 analysis/gsea/Common Genes/New Comparisons/Without trop2'
# create & save html files for each gene table (containing total gene counts across gene sets)
setwd(main_folder)
html_files <- generate_html_geneTables(final_results, "")
...
link_headers_to_files <- function(report_html_path, folder_path) {
  ...
  for (header_node in header_nodes) {
    ...
    for (pattern in file_patterns) {
      html_file_name <- paste0(reg_type, "_", pattern, "_genes.html")
      html_file_path <- file.path(folder_path, clean_header_text, html_file_name)
...
report_html_files <- list.files(path = folder_path, pattern = "GeneTable", full.names = TRUE, recursive = TRUE)
